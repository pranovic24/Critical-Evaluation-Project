...
for i = 1:row-1
    currentData(i,:) = log(before(i+1,:)./before(i,:));
end
...
title('Empirical Eigenvalue Distribution (Before Crisis)');
xlabel('\lambda');
ylabel('\rho(\lambda)');
...
hold on;
plot(d,P_rcm,'-r','LineWidth',2);
title('Empirical Eigenvalue Distribution of C_{shuffle} (Before Crisis)');
xlabel('\lambda');
ylabel('\rho(\lambda)');
legend('Eigenvalues Distribution \rho(\lambda) ','\rho_{rm}(\lambda)');
set(gcf,'color','w');
...
hold on;
plot(d,P_rcm,'-r','LineWidth',2);
title('Eigenvalue Distribution Market Mode Removal (Before Crisis)');
xlabel('\lambda');
ylabel('\rho(\lambda)');
legend('Eigenvalues Distribution \rho(\lambda) ','\rho_{rm}(\lambda)');
set(gcf,'color','w');
...
hold off;
title(['Largest Eigenvector u_{',num2str(k),'} Distribtuion of C_{shuffle} (Before Crisis)']);
